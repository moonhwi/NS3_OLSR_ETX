  ...
  namespace olsr
  {
          ...
          for (LinkSet::const_iterator it2 = linkSet.begin(); it2 != linkSet.end(); it2++)
          {
                ...
                link_tuple.time >= Simulator::Now())
            {
              ...
              AddEntry(link_tuple.neighborIfaceAddr, link_tuple.neighborIfaceAddr,
                       link_tuple.localIfaceAddr, 1, link_tuple.Etx);
        ...
        for (TopologySet::const_iterator it = topology.begin(); it != topology.end(); it++)
        {
          ...
          if (!have_destAddrEntry && have_lastAddrEntry && lastAddrEntry.distance == h)
          {
            ...
            AddEntry(topology_tuple.destAddr, lastAddrEntry.nextAddr, lastAddrEntry.interface,
                     h + 1, lastAddrEntry.etxDistance + topology_tuple.Etx);
            added = true;
          }
          else
          {
    ...
    RoutingProtocol::SendHello(void)
    {
           ...
           link_tuple++)
      {
        ...
        std::vector<uint32_t> helloReceived;
        std::map<ns3::Ipv4Address, uint32_t> m_HelloReceived = m_state.GetHelloReceived();
        for (std::vector<Ipv4Address>::iterator it = linkMessage.neighborInterfaceAddresses.begin();
             it != linkMessage.neighborInterfaceAddresses.end(); it++)
        {
        ...
        linkMessage.neighborEtxs.insert(linkMessage.neighborEtxs.end(), helloReceived.begin(),
                                        helloReceived.end());
                                 ...
                                 const Ipv4Address &receiverIface, const Ipv4Address &senderIface)
    {
      ...
      if (link_tuple == NULL)
      {
        ...
        if (m_HelloReceived.find(senderIface) == m_HelloReceived.end())
        {
          m_HelloReceived.insert(std::pair<Ipv4Address, uint32_t>(senderIface, 0));
        }
        m_HelloReceived[senderIface] += 1;
        newLinkTuple.Etx = (float)1 / (m_HelloReceived[senderIface]);
        m_state.setHelloReceived(m_HelloReceived);
...
OLSR::mac_failed (Ptr<Packet> p)
{
  ...
  drop (p, DROP_RTR_MAC_CALLBACK);
    ...
    void RoutingProtocol::updateEtxDataTable(const Ipv4Address &src,
    ...
    RoutingProtocol::AddEntry(Ipv4Address const &dest, Ipv4Address const &next, uint32_t interface,
                              uint32_t distance, float etxDistance)
    {
    ...
    RoutingProtocol::AddEntry(Ipv4Address const &dest, Ipv4Address const &next,
                              Ipv4Address const &interfaceAddress, uint32_t distance, float etxDistance)
    {
